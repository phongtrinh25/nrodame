...
public class DataGame {
    ...
    public static byte vsMap = 70;
    public static byte vsSkill = 6;
    public static byte vsItem = 70;
    public static int vsRes = 752012;

    public static String LINK_IP_PORT = "NRO KING:14.225.217.62:14445:0,0,0";
    private static final String MOUNT_NUM = "733:1,734:2,735:3,743:4,744:5,746:6,795:7,849:8,897:9,920:10,1143:11,1141:15,1377:20,1378:21,1270:23,1271:25,1272:26,1273:27,1274:19";
    ...
    public static void updateMap(MySession session) {
        ...
        try {
            ...
            for (MobTemplate temp : Manager.MOB_TEMPLATES) {
                ...
                msg.writer().writeUTF(temp.name);
                msg.writer().writeInt(Util.CuongGH(temp.hp));
    ...
    public static void effData(MySession session, int id, int... idtemp) {
        ...
        Message msg;
        try {
            final byte[] effData = FileIO.readFile("data/girlkun/effect/x" + session.zoomLevel + "/data/DataEffect_" + idT);
...
//            }
    ...
    public static void requestMobTemplate2(MySession session, int id) {
        Message msg;
        try {
            byte[] data = FileIO.readFile("data/girlkun/mob/x" + session.zoomLevel + "/Data/" + id);
            byte[] img = FileIO.readFile("data/girlkun/mob/x" + session.zoomLevel + "/Image/" + id + ".png");
            msg = new Message(11);
            ...
            msg.writer().write(data);
            msg.writer().writeInt(img.length);
            msg.writer().write(img);
            session.sendMessage(msg);
            msg.cleanup();
        } catch (Exception e) {
        }
...
//        }
    ...
    public static void requestMobTemplateAll(MySession session, int id) {
        Message msg;
        try {
            byte[] mob = FileIO.readFile("data/girlkun/mob/x" + session.zoomLevel + "/" + id);
            msg = new Message(11);
...
//        }
    ...
    public static void edt(MySession session, int id, int... idtemp) {
        if (id >= 205 && id <= 210) {
            sendEffectTemplate(session, id);
            return;
        } else if (id == 66 || id == 68 || id == 73 || id == 74 || id <= 59 || id == 87 || id >= 0) {
            int idT = id;
            if (idtemp.length > 0 && idtemp[0] != 0) {
                idT = idtemp[0];
            }
            Message msg;
            try {
                final byte[] effData = FileIO.readFile("nbproject/private/New folder/test/x" + session.zoomLevel + "/data/DataEffect_" + idT);
                byte[] effImg = FileIO.readFile("nbproject/private/New folder/test/x" + session.zoomLevel + "/img/ImgEffect_" + idT + ".png");
                msg = new Message(-66);
                msg.writer().writeShort(id);
                msg.writer().writeInt(effData.length);
                msg.writer().write(effData);
