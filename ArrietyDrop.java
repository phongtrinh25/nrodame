...
public class ArrietyDrop {
    ...
    public static void DropItemReWard(Player player, int idItem, int soluong, int x, int y) {
        ItemMap item = new ItemMap(player.zone, idItem, soluong, Util.nextInt((x - 0), (x + 0)), y, player.id);
    ...
    public static Item randomCS_DHD(int itemId, int gender) {
        ...
        if (gang.contains(itemId)) {
            it.itemOptions.add(new Item.ItemOption(0, Util.highlightsItem(gender == 2, new Random().nextInt(150) + 8800))); // 8500-10000
        }
