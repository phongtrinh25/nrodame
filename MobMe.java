...
public final class MobMe extends Mob {
    ...
    public MobMe(Player player) {
        ...
        long largeNumber =  player.nPoint.hpMax;
        int intValue = (int) Math.max(Math.min(largeNumber, Integer.MAX_VALUE), Integer.MIN_VALUE);
        if (intValue < 0) {
            intValue = 0;
        }
        this.point.maxHp = SkillUtil.getHPMobMe(intValue, level);
        this.point.dame = SkillUtil.getHPMobMe((int) player.nPoint.getDameAttack(false), level);
        if (this.player.setClothes.pikkoroDaimao == 5) {
            this.point.dame *= 1;
        }        
        if (this.player.setClothes.pikkoroDaimao_level_1 == 5) {
            this.point.dame *= 2;
        }
        if (this.player.setClothes.pikkoroDaimao_level_2 == 5) {
            this.point.dame *= 3;
        }
        ...
        try {
            if (pl != null) {
                if (pl.nPoint.hp > this.point.dame && pl.nPoint.hp > pl.nPoint.hpMax * 0.05) {
                    double dameHit = pl.injured2(null, this.point.dame, true, true);
                    msg = new Message(-95);
                    ...
                    msg.writer().writeInt(Util.CuongGH(pl.nPoint.hp));
            ...
            if (mob != null) {
                if (!mob.isDie()) {
                    if (!mob.isMobMe) {
                        ...
                        if (mob.tempId == 0 && mob.zone.map.mapId != 179) {
                            tnsm = 1;
                            ...
                            msg.writer().writeInt(Util.CuongGH(mob.point.gethp()));
                            msg.writer().writeInt(Util.CuongGH(this.point.dame));
                        }
    ...
    public void spawn() {
        ...
        try {
            ...
            msg.writer().writeInt(Util.CuongGH(this.point.hp));// hp mob
            Service.gI().sendMessAllPlayerInMap(this.zone, msg);
            msg.cleanup();
        } catch (Exception e) {
