...
import static com.girlkun.server.Manager.CAITRANG;
import static com.girlkun.server.Manager.ITEM_TEMPLATES;
    ...
    private static void loadItemShop(Connection con, TabShop tabShop) {
        try {
            ...
            while (rs.next()) {
                ...
                if(itemShop.temp.type == 5 && "SANTA".equals(itemShop.tabShop.shop.tagName)) CAITRANG.add(ItemService.gI().createNewItem(itemShop.temp.id));           
            }
            try {
                if (rs != null) {
                    rs.close();
