...
public class ShopServiceNew {
    ...
    private boolean subMoneyByItemShop(Player player, ItemShop is) {
        ...
        int coupon = 0;
         int thucan= 0;
         int[] itemIds = {663, 664, 665, 666, 667};
         List<Item> items = new ArrayList<>();
         Item item = null;
        switch (is.typeSell) {
            case COST_GOLD:
                gold = is.cost;
                if ("BILL".equals(is.tabShop.shop.tagName)) {
                    thucan = 99;                   
                }
                break;
            case COST_GEM:
        ...
        for (int id : itemIds) {
            item = InventoryServiceNew.gI().findItemBag(player, id);
            if (item != null && item.quantity >= thucan) {
            items.add(item); 
          }
        }
        if (player.inventory.gold < gold) {
            ...
            return false;
        }  else if ("BILL".equals(is.tabShop.shop.tagName)) {
            if (items.isEmpty()) {
            Service.gI().sendThongBao(player, "Bạn không có đủ thức ăn");
            return false;
            }
        }
        if (!items.isEmpty()) {
            Random rnd = new Random();
            Item itemToRemove = items.get(rnd.nextInt(items.size())); 
            InventoryServiceNew.gI().subQuantityItemsBag(player, itemToRemove, thucan);
   ...
   public void showConfirmSellItem(Player pl, int where, int index) {
        if (index == 2 || index == 1) {
            ...
            if (item != null && item.isNotNullItem() && item.template.id == 457) {
                NpcService.gI().createMenuConMeo(pl, ConstNpc.BAN_NHIEU_THOI_VANG, 7710,
                        ...
                        + "|-1|Theo nguyện vọng góp ý từ các chiến binh, ta được Admin Gọi Đến để\n"
        ...
        if (item != null && item.isNotNullItem()) {
            ...
            if (item.template.id == 457 || item.template.id== 1322) {
                quantity = 1;
            } else {
                cost /= 4;
            }
                ...
                if (item.isNotNullItem()) {
                    if (InventoryServiceNew.gI().getCountEmptyBag(player) != 0) {
                        ...
                        Service.gI().sendThongBao(player,
                                "Bạn nhận được " + (item.template.id == 861
                                        ? Util.numberToMoney(item.quantity) + "  Hồng Ngọc" : item.template.name));
                        InventoryServiceNew.gI().sendItemBags(player);
                        items.remove(index);
                    } else {
                        Service.gI().sendThongBao(player, "Hành trang đã đầy");
